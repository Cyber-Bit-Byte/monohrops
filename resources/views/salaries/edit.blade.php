...
@section('page-title', __('Edit Salary'))
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form action="{{ route('salaries.update', $salary) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input id="amount" name="amount" type="number" step="0.01" value="{{ old('amount', $salary->amount) }}" class="mt-1 block w-full" required>
                    ...
                    <div class="mb-4">
                        ...
                        <input id="date" name="date" type="date" value="{{ old('date', $salary->date->format('Y-m-d')) }}" class="mt-1 block w-full" required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="notes" name="notes" class="mt-1 block w-full">{{ old('notes', $salary->notes) }}</textarea>
                        @error('notes') <div class="text-red-500">{{ $message }}</div> @enderror
                    </div>

                    <button type="submit" class="bg-cyan-500 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded">{{ __('Update Salary') }}</button>
                </form>
            </div>
        </div>
    </div>
