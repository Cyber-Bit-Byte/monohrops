    ...
    <div class="space-y-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <div class="flex gap-4 mb-4">
                    <a href="{{ route('costs.create') }}" class="bg-cyan-500 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded">{{ __('Add Cost') }}</a>
                </div>
                <form method="GET" action="{{ route('costs.index') }}" class="flex flex-wrap gap-4 items-end">
                    <div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_from" value="{{ request('date_from', now()->startOfMonth()->format('Y-m-d')) }}" class="border rounded px-2 py-1 dark:bg-gray-700 dark:border-gray-600">
                    </div>
                    <div>
                        <label class="block text-sm font-medium">{{ __('Date To') }}</label>
                        <input type="date" name="date_to" value="{{ request('date_to', now()->endOfMonth()->format('Y-m-d')) }}" class="border rounded px-2 py-1 dark:bg-gray-700 dark:border-gray-600">
                    </div>
                    <button type="submit" class="bg-cyan-500 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded">{{ __('Filter') }}</button>
                    <a href="{{ route('costs.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Reset</a>
                </form>
            </div>
        </div>
                     ...
                     <tbody>
                         @forelse($cost_list as $cost)
                             <tr>
                                 ...
                                 <td class="border px-4 py-2">
                                     <a href="{{ route('costs.edit', $cost) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-sm">{{ __('Edit') }}</a>
                                     <form action="{{ route('costs.destroy', $cost) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this cost entry?');">
                                         @csrf
                                         @method('DELETE')
                                         <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded text-sm">{{ __('Delete') }}</button>
                                     </form>
                                 </td>
                             </tr>
                         @empty
                             <tr>
                                 <td colspan="7" class="border px-4 py-2 text-center">No costs found.</td>
                             </tr>
                         @endforelse
                     </tbody>
                 </table>
