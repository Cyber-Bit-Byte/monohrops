...
@section('page-title', __('Edit Earnings'))
                ...
                <form action="{{ route('earnings.update', $earning) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="employee_id" name="employee_id" class="mt-1 block w-full" required>
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" {{ old('employee_id', $earning->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="department_id" name="department_id" class="mt-1 block w-full">
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" {{ old('department_id', $earning->department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="type" name="type" class="mt-1 block w-full" required>
                            ...
                            <option value="Account receivable" {{ old('type', $earning->type) == 'Account receivable' ? 'selected' : '' }}>Account Receivable</option>
                            <option value="Cash" {{ old('type', $earning->type) == 'Cash' ? 'selected' : '' }}>Cash</option>
                            <option value="Advance" {{ old('type', $earning->type) == 'Advance' ? 'selected' : '' }}>Advance</option>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input id="amount" name="amount" type="number" step="0.01" value="{{ old('amount', $earning->amount) }}" class="mt-1 block w-full" required>
                    ...
                    <div class="mb-4">
                        ...
                        <input id="date" name="date" type="date" value="{{ old('date', $earning->date->format('Y-m-d')) }}" class="mt-1 block w-full" required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="notes" name="notes" class="mt-1 block w-full">{{ old('notes', $earning->notes) }}</textarea>
                        @error('notes') <div class="text-red-500">{{ $message }}</div> @enderror
                    </div>

                    <button type="submit" class="bg-cyan-500 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded">{{ __('Update Earning') }}</button>
                </form>
            </div>
        </div>
    </div>
